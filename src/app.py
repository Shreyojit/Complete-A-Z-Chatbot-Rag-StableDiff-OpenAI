        ...
        with gr.TabItem("Advanced Multimodal ChatBot"):
            with gr.Row() as app_row:
                with gr.Column(scale=1) as left_column:
                    ...
                    with gr.Accordion("LLM Parameters", open=False):
                        gr.Markdown("(Will be applied whenever it is applicable)")
                        gpt_temperature = gr.Slider(
                            minimum=0, maximum=1, value=0, step=0.1,
                            label="Temperature:", info="Use 0 for RAG. 1 makes model more creative.",
                            interactive=True
                        )
                    ...
                    with gr.Accordion("RAG Parameters", open=False):
                        rag_top_k_retrieval = gr.Slider(
                            minimum=0, maximum=5, value=2, step=1, interactive=True,
                            label="Top K:", info="Number of retrieved chunks for RAG"
                        )
                ...
                with gr.Column(scale=8) as right_column:
                    with gr.Row() as row_one:
                        with gr.Column(visible=False) as reference_bar:
                            ref_output = gr.Markdown(label="RAG Reference Section")
                        with gr.Column() as chatbot_output:
                            chatbot = gr.Chatbot(
                                ...
                                height=500,
                                avatar_images=("images/AI_RT.png", "images/chatbot.png"),
                            )
                        with gr.Column(visible=False) as full_image:
                            image_output = gr.Image()
                            chatbot.like(UISettings.feedback, None, None)
                    with gr.Row():
                        input_txt = gr.MultimodalTextbox(
                            interactive=True,
                            lines=2,
                            file_types=["image"],
                    ...
                    with gr.Row() as row_two:
                        upload_btn = gr.UploadButton(
                            "📁 Upload PDF or doc files for RAG",
                            file_types=['.pdf', '.doc'],
                            file_count="multiple"
                        )
                        ...
                        btn_toggle_sidebar.click(
                            fn=UISettings.toggle_sidebar,
                            inputs=[reference_bar_state],
                            outputs=[reference_bar, reference_bar_state]
                        )
            ...
            txt_msg = input_txt.submit(
                ...
                inputs=[
                    ...
                    app_functionality,
                    gpt_temperature,
                    llava_max_output_token,
                    input_audio_block,
                    rag_top_k_retrieval,
                    rag_search_type
                ],
                outputs=[chatbot, input_txt, ref_output],
                queue=False
            ).then(
                lambda: gr.Textbox(interactive=True),
                None, [input_txt], queue=False
            ...
            txt_msg = audio_submit_btn.click(
                fn=ChatBot.respond,
                inputs=[
                    chatbot,
                    input_txt,
